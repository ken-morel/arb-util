use crate::{
    arb::ArbFile,
    utils::{id_string, localization_getter},
};
...
fn extract(p: &Project, file: PathBuf) {
    let main_arb = ArbFile::new(p.arb_template_path());
    if let Ok(content) = std::fs::read_to_string(&file) {
            ...
            .rev()
        {
            ...
            if let Err(e) = main_arb.add_key(&id, &content) {
                println!(
...
pub fn start(p: Project) -> Result<(), String> {
    for path in DirWatcher::new(p.root_dir.join("lib").as_path())?.flatten() {
